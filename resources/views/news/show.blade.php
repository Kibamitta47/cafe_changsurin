...
<body class="bg-slate-50">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        {{-- สร้าง Layout 2 คอลัมน์ --}}
        <div class="flex flex-col lg:flex-row lg:gap-8">

            <!-- ========= คอลัมน์ซ้าย: เนื้อหาข่าวหลัก (Main Content) ========= -->
            <main class="w-full lg:w-2/3 bg-white p-6 sm:p-8 rounded-2xl shadow-sm border border-slate-200">
                
                {{-- หัวข้อ --}}
                <h1 class="text-3xl md:text-4xl font-bold text-slate-900 mb-4">{{ $newsItem->title }}</h1>
                ...
                @if(!empty($newsItem->images))
                    <div class="mb-6 rounded-xl overflow-hidden">
                        <img src="{{ asset('storage/' . $newsItem->images[0]) }}" alt="{{ $newsItem->title }}" class="w-full h-auto object-cover">
                    </div>
                @endif
                        ...
                        <div class="grid grid-cols-2 md:grid-cols-3 gap-4">
                            ...
                            @foreach(array_slice($newsItem->images, 1) as $imageFile)
                                <div class="aspect-w-1 aspect-h-1">
                                    <img src="{{ asset('storage/' . $imageFile) }}" alt="{{ $newsItem->title }}" class="w-full h-full object-cover rounded-lg border">
                                </div>
                            @endforeach
                        </div>
                    </div>
                ...
                <div class="mt-8 pt-6 border-t border-slate-200">
                    <a href="{{ $newsItem->link_url }}" target="_blank" rel="noopener noreferrer" class="inline-flex items-center text-cyan-600 hover:text-cyan-800 hover:underline transition-colors duration-200 font-semibold">
            ...
            <!-- ========= คอลัมน์ขวา: ข่าวสารแนะนำ (Sidebar) ========= -->
