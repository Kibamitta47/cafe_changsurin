                ...
                <section class="bg-white/90 backdrop-blur-sm shadow-2xl rounded-2xl p-6 md:p-8">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-4 text-slate-700">
                        ...
                        <div class="flex items-center">
                            <i class="fa-solid fa-tags text-cyan-500 w-5 mr-3 shrink-0"></i>
                            <span><strong>ช่วงราคา:</strong> {{ $cafe->price_range ?? '-' }}</span>
                        </div>

                        {{-- โทรศัพท์ --}}
                        <div class="flex items-center">
                            <i class="fa-solid fa-phone text-cyan-500 w-5 mr-3 shrink-0"></i>
                            <span><strong>เบอร์โทรศัพท์:</strong> {{ $cafe->phone ?? '-' }}</span>
                        </div>
                    ...
                    <div class="mt-6 pt-6 border-t border-slate-200">
                        <h3 class="text-lg font-semibold mb-2 flex items-center">
                            <i class="fa-solid fa-circle-info mr-2 text-cyan-500"></i>
                            รายละเอียดเพิ่มเติม
                        </h3>
                    ...
                    {{-- Helper: แปลงคอลัมน์เป็นอาเรย์อย่างปลอดภัย --}}
                    @php
                        ...
                        $amenities       = $toArray($cafe->amenities);
                    ...
                    @if(!empty($amenities))
                        <div class="mt-6 pt-6 border-t border-slate-200">
                            <h3 class="text-lg font-semibold mb-3 flex items-center">
                                <i class="fa-solid fa-wifi mr-2 text-cyan-500"></i>
                                สิ่งอำนวยความสะดวก
                            </h3>
                    ...
                    @if(!empty($cafeStyles))
                        <div class="mt-6 pt-6 border-t border-slate-200">
                            <h3 class="text-lg font-semibold mb-3 flex items-center">
                                <i class="fa-solid fa-palette mr-2 text-purple-500"></i>
                                สไตล์คาเฟ่
                            </h3>
                            <div class="flex flex-wrap gap-2">
                    ...
                    @if(!empty($paymentMethods))
                        <div class="mt-6 pt-6 border-t border-slate-200">
                            <h3 class="text-lg font-semibold mb-3 flex items-center">
                                <i class="fa-regular fa-credit-card mr-2 text-green-500"></i>
                                ช่องทางชำระเงิน
                            </h3>
                            <div class="flex flex-wrap gap-2">
                    ...
                    @if(!empty($otherServices))
                        <div class="mt-6 pt-6 border-t border-slate-200">
                            <h3 class="text-lg font-semibold mb-3 flex items-center">
                                <i class="fa-solid fa-bell-concierge mr-2 text-indigo-500"></i>
                                บริการเพิ่มเติม
                            </h3>
                            <div class="flex flex-wrap gap-2">
                ...
                @if (!empty($cafeImages))
                    <section class="bg-white/90 backdrop-blur-sm shadow-2xl rounded-2xl p-6 md:p-8">
                        <h2 class="text-2xl font-bold mb-5 flex items-center">
                            <i class="fa-solid fa-images mr-3 text-amber-500"></i>
                            แกลเลอรี
                        </h2>
                ...
                <section class="bg-white/90 backdrop-blur-sm shadow-2xl rounded-2xl p-6 md:p-8">
                    <div class="flex justify-between items-center mb-6">
                        ...
                        @auth
                            ...
                            <a
                                href="{{ route('user.reviews.create', ['cafe_id' => $cafe->cafe_id]) }}"
                            ...
                            @foreach ($reviews as $review)
                                <div class="border-t border-slate-200 pt-6 first:border-t-0 first:pt-0">
                                    ...
                                    @if(!empty($review->content))
                                        <p class="mt-1 text-slate-700 whitespace-pre-line leading-relaxed">
                                            {{ $review->content }}
                                        </p>
                                    @endif
                                    ...
                                    {{-- รูปภาพรีวิว --}}
                                    @php
                                        $revImages = is_string($review->images)
