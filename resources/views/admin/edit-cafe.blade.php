...
<head>
    ...
    <style>
        ...
        :root {
            --primary-blue: #3b82f6; --dark-blue: #2563eb; --gray-light: #e5e7eb;
            --white: #fff;
        }
        body { font-family: 'Inter', sans-serif; background:#f0f2f5; color:#333; padding:20px 0; transition: padding-left .3s; }
        ...
        @media (max-width:768px){ .container.mt-5.mb-5{padding:20px;} .btn-check + .btn{display:block;width:100%;margin-bottom:.5rem!important;} .d-flex.justify-content-end.gap-2.mt-3{flex-direction:column} .d-flex.justify-content-end.gap-2.mt-3 .btn{width:100%} }
    </style>
</head>
<body>
...
<script>
/* ====== จำกัดจำนวนรูป และตรวจ validate พิกัด ====== */
...
if (latInput.value && lngInput.value) {
  ...
  if (!mueangSurinBounds.contains(initial)) displayOutOfBoundsWarning(true);
  else checkCoordinates(initial.lat, initial.lng);
}

L.Control.geocoder({
  defaultMarkGeocode:false, placeholder:"ค้นหาที่อยู่...", collapsed:true, suggestMinLength:3,
...
function displayOutOfBoundsWarning(show){
  if (show) { outOfBoundsWarning.classList.remove('d-none'); submitBtn.disabled=true; }
  else { outOfBoundsWarning.classList.add('d-none'); }
}

async function checkCoordinates(lat,lng){
  const currentCafeId = "{{ $cafe->id ?? 'null' }}";
  if (isNaN(lat) || isNaN(lng)) { duplicateCoordsWarning.classList.add('d-none'); submitBtn.disabled=true; return; }
  try{
    ...
    if (isDup) { duplicateCoordsWarning.classList.remove('d-none'); submitBtn.disabled=true; }
    else { duplicateCoordsWarning.classList.add('d-none'); if (outOfBoundsWarning.classList.contains('d-none')) submitBtn.disabled=false; }
  } catch(e){
...
<!-- ============= บีบอัดรูปก่อนอัปโหลด (หลายไฟล์) ============= -->
<script>
document.addEventListener('DOMContentLoaded', () => {
  const MAX_FILES   = 5;                 // จำนวนรูปสูงสุด
  const MAX_BYTES   = 2.5 * 1024 * 1024; // ขนาดสูงสุด/รูป (2.5MB) ปรับตาม validation ฝั่งเซิร์ฟเวอร์
  const MAX_DIM     = 1600;              // ด้านยาวสุด
  const MIN_QUALITY = 0.5;               // คุณภาพต่ำสุด
  ...
  async function compressImage(file){
    ...
    let q = 0.85;
    let blob = await canvasToBlob(canvas, TARGET_TYPE, q);
    while (blob.size > MAX_BYTES && q > MIN_QUALITY) {
      q = Math.max(MIN_QUALITY, q - 0.08);
      blob = await canvasToBlob(canvas, TARGET_TYPE, q);
    }

    const base = (file.name || 'image').replace(/\.[^.]+$/, '');
    const ext = TARGET_TYPE === 'image/webp' ? 'webp' : 'jpg';
    return new File([blob], `${base}.${ext}`, { type: TARGET_TYPE, lastModified: Date.now() });
  }

  async function compressAllIfNeeded(fileList){
    const dt = new DataTransfer();
    for (const f of fileList) {
      dt.items.add(await compressImage(f));
    }
  ...
  input.addEventListener('change', async () => {
    if (input.files.length > MAX_FILES) {
      alert(`เลือกได้สูงสุด ${MAX_FILES} รูปภาพเท่านั้น`);
      input.value = '';
      return;
    }
  ...
  cafeForm.addEventListener('submit', async (e) => {
    if (input.files.length > MAX_FILES) {
      e.preventDefault();
      alert(`กรุณาอัปโหลดรูปภาพไม่เกิน ${MAX_FILES} รูปเท่านั้น`);
      return;
    }
    const need = Array.from(input.files).some(f => f.size > MAX_BYTES);
    if (need) {
      e.preventDefault();
      await compressAllIfNeeded(input.files);
      if (Array.from(input.files).some(f => f.size > MAX_BYTES)) {
        alert('มีไฟล์ที่ยังใหญ่เกินกำหนด กรุณาเลือกรูปที่เล็กลง');
        return;
      }
      cafeForm.submit();
    }
